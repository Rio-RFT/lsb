...
public:
	...
	virtual void SetLocale(const std::string& locale) override
	{
		...
		RegSetKeyValueW(HKEY_CURRENT_USER, L"SOFTWARE\\LSB", L"Global Locale", REG_SZ, wPath.c_str(), (wPath.size() * 2) + 2);
...
private:
	void InitDefaultLocale()
	{
		...
		RegGetValueW(HKEY_CURRENT_USER, L"SOFTWARE\\LSB", L"Global Locale", RRF_RT_REG_SZ, NULL, regPath, &size);
